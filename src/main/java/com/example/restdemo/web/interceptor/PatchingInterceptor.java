package com.example.restdemo.web.interceptor;
...
public class PatchingInterceptor implements ReaderInterceptor {
    ...
    public Object aroundReadFrom(ReaderInterceptorContext readerInterceptorContext) throws IOException {
        ...
        for (Method next : resource.getClass().getMethods()) {
            if (next.getAnnotation(GET.class) != null && next.getName().matches("^get.*ById$")) {
                found = next;
                break;
            }
        }
        ...
        if (found != null) {
            ...
            Object bean;
            try {
                Response getResp = (Response) found.invoke(resource, info.getPathParameters().getFirst("id"));
                bean = getResp.getEntity();
            } catch (Exception e) {
            ...
            bodyWriter.writeTo(bean, bean.getClass(), bean.getClass(),
                    new Annotation[0], MediaType.APPLICATION_JSON_TYPE, new MultivaluedHashMap<>(), bout);
            ...
            JsonNode serverState = mapper.readValue(bout.toByteArray(), JsonNode.class);
